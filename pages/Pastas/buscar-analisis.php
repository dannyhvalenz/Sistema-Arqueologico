    ...
    if ($result == true){
        if(mysqli_num_rows($result) > 0) {
            ...
            while($row = mysqli_fetch_array($result)){
                ...
                $directorio = 'imagenes/'.$row["IdAnalisisPastas"];
                if (is_dir($directorio)){
                    $output .= '<td><a onclick="agregarImagenes('.$row["IdAnalisisPastas"].')" value="'.$row["IdAnalisisPastas"].'"data-toggle="modal" data-toggle="modal" data-target=".bs-example-modal-lg" data-id="'.$row["IdAnalisisPastas"].'" class="btn btn-primary  glyphicon glyphicon-picture" id="'.$row["IdAnalisisPastas"].'"></a></td>';
                }else{
                    $output .= '<td><a onclick="agregarImagenes('.$row["IdAnalisisPastas"].')" value="'.$row["IdAnalisisPastas"].'"data-toggle="modal" data-toggle="modal" data-target=".bs-example-modal-lg" data-id="'.$row["IdAnalisisPastas"].'" class="btn btn-primary  glyphicon glyphicon-picture" id="'.$row["IdAnalisisPastas"].'" disabled></a></td>';
                }            
